...
function WorkspaceInfo({ workspaceId }) {
  ...
  const [workspaceAddress, setWorkspaceAddress] = useState(
    workspaces[workspaceId].address
  );
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell align="right" className={classes.tabHeader}>
              Date
            </TableCell>
            <TableCell align="right" className={classes.tabHeader}>
              Hour
          ...
          {Object.keys(reservationsForWorkspace).map((reservationId) => {
            return (
              <TableRow key={reservationId}>
                <TableCell>
                  {
                    users[reservationsForWorkspace[reservationId].employeeId]
                      .login
                  }
                </TableCell>
                ...
                <TableCell align="right">
                  {reservationsForWorkspace[reservationId].beginHour} -{" "}
                  {reservationsForWorkspace[reservationId].beginHour +
                    reservationsForWorkspace[reservationId].period}
                </TableCell>
              </TableRow>
            );
          })}
        ...
        <Grid item xs={4}>
          <Card className={classes.card}>
            <CardContent>
              ...
              <ReservationList workspaceId={workspaceId} />
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={5}>
